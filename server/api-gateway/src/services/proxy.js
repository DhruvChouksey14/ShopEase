...
const circuitBreakers = {
     ...
     catalogService: new CircuitBreaker('catalog-service'),
     ...
     cartService: new CircuitBreaker('cart-service'),
};
...
async function forwardRequest(serviceUrl, path, method, data, headers, circuitBreaker) {
     ...
     try {
          ...
          console.error(`Error forwarding to ${serviceUrl}:`, {
               message: err.message,
               code: err.code,
               url: url,
               method: method,
          ...
          if (err.response) {
               console.error(`Service error from ${serviceUrl}:`, {
          ...
          console.error(`Network error while calling ${serviceUrl}:`, err.message);
